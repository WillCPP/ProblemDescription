...
def base_model_sa(save_model=False, model_folder=None, use_np_noise_privacy=False, noise_multiplier=None):
    ...
    if use_np_noise_privacy:
        ...
        sensitivity = 1
        epsilon = 0.1
        laplace_rng = np.random.default_rng()
        result = laplace_rng.laplace(scale=sensitivity/epsilon, size=[60000,28,28,1])
        # result = np.random.laplace(scale=sensitivity/epsilon, size=[60000,28,28,1])
        result = np.rint(result)

        perturbed_data = x_train + result
